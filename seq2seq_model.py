...
class Seq2SeqModel(object):
  ...
  def __init__(self, source_vocab_size, target_vocab_size, buckets, size,
    ...
    if 0 < num_samples < self.target_vocab_size:
      with tf.device("/cpu:0"):
        w = tf.get_variable(name='projection_w', shape=[size, self.target_vocab_size])
        w_t = tf.transpose(w)
        b = tf.get_variable(name='projection_b', shape=[self.target_vocab_size])
      ...
      def sampled_loss(logits, labels):
        with tf.device("/cpu:0"):
          ...
          return tf.nn.sampled_softmax_loss(weights=w_t, biases=b,
  ...
  def step(self, session, encoder_inputs, decoder_inputs, target_weights,
    ...
    # Collect the feed dict: encoder inputs, decoder inputs, target_weights, as provided.
    feed_dict = {}
